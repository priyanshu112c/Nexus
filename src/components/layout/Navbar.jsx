...
export default function Navbar() {
    ...
    return (
        <>
            ...
            <div className="fixed top-0 inset-x-0 z-[70] h-9 bg-gradient-to-r from-neon/20 via-neon-purple/20 to-neon/20 backdrop-blur-xl border-b border-ink/5 flex items-center justify-center gap-2 text-xs">
                <span className="inline-block w-2 h-2 rounded-full bg-neon animate-pulse-glow" />
                <span className="tracking-wide text-ink/80">Summer Tech Festival — Up to 40% off flagship devices</span>
                <Link to="/deals" className="text-neon-light hover:underline hidden sm:inline">Shop now</Link>
            </div>
            ...
            >
                <div className="max-w-[1400px] mx-auto px-6 flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        {/* Profile */}
                        <div className="relative hidden sm:block" onMouseEnter={() => setProfileHover(true)} onMouseLeave={() => setProfileHover(false)}>
                        ...
                        {/* Notifications */}
                        <button className="relative w-10 h-10 rounded-full glass flex items-center justify-center text-ink/70 hover:text-ink transition-colors duration-300" aria-label="Notifications" data-hover>
                        ...
                        <button
                            onClick={toggleTheme}
                            className="relative w-10 h-10 rounded-full glass flex items-center justify-center text-ink/70 hover:text-ink hover:shadow-glow transition-all duration-300 overflow-hidden"
                ...
                <AnimatePresence>
                    {mobileOpen && (
                        <motion.nav
                            ...
                            className="lg:hidden glass-strong overflow-hidden border-t border-ink/5"
                        >
                            <div className="px-6 py-4 flex flex-col gap-1">
                                {navLinks.map((link) => (
                                    <NavLink
                                        ...
                                        className={({ isActive }) =>
                                            `px-4 py-3 rounded-xl text-sm transition-colors ${isActive ? 'bg-ink/5 text-ink' : 'text-ink/60 hover:text-ink hover:bg-ink/5'
                                            }`
                                        }
                                    >
                                        {link.label}
