...
export default function About() {
    return (
        <div className="max-w-[1400px] mx-auto px-6 pt-40 pb-20">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-24">
                {stats.map((stat, i) => (
                    <motion.div
                        ...
                        className="glass rounded-3xl p-8 text-center hover:shadow-glow transition-shadow duration-500"
                        data-hover
                    >
                        <p className="font-display text-4xl md:text-5xl font-bold text-gradient-static mb-2">{stat.value}</p>
                        <p className="text-sm text-ink/40">{stat.label}</p>
                    </motion.div>
                ))}
            </div>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-24">
                {values.map(({ icon: Icon, title, text }, i) => (
                    <motion.div
                        ...
                        viewport={{ once: true, margin: '-60px' }}
                        transition={{ duration: 0.6, delay: i * 0.1 }}
                        className="relative group glass rounded-3xl p-8 hover:border-neon/20 hover:shadow-glow transition-all duration-500"
                        data-hover
                    >
                        ...
                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-neon to-neon-purple flex items-center justify-center mb-5 shadow-glow">
            ...
            {/* Brands */}
            <div className="glass-strong rounded-[2.5rem] p-12 text-center">
                <h2 className="font-display text-3xl md:text-4xl font-bold mb-4">
                    Powering The <span className="text-gradient">Future</span>
                </h2>
                <p className="text-ink/40 max-w-xl mx-auto mb-10">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {brands.map((brand, i) => (
                        <motion.div
                            ...
                            className="group flex items-center justify-center gap-3 p-4 rounded-2xl glass hover:border-neon/30 transition-all duration-300"
                            data-hover
                        >
                            <img src={brand.image} alt={brand.name} className="w-10 h-10 rounded-xl object-cover opacity-70 group-hover:opacity-100 transition-opacity" />
                            <span className="font-display font-semibold text-sm group-hover:text-neon-light transition-colors">{brand.name}</span>
                        </motion.div>
                    ))}
                </div>
            </div>
