                        ...
                        <AnimatePresence initial={false}>
                            {cart.map((item) => (
                                <motion.div
                                    ...
                                    className="group relative glass rounded-3xl p-4 hover:border-neon/20 hover:shadow-glow transition-all duration-300"
                                >
                                    <div className="flex gap-5">
                                        <Link to={`/product/${item.id}`} className="shrink-0">
                                            <div className="w-28 h-28 md:w-32 md:h-32 rounded-2xl overflow-hidden">
                                                <img
                                                    src={item.image}
                                                    alt={item.name}
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-start justify-between gap-4">
                                                <div>
                                                    <Link to={`/product/${item.id}`}>
                                                        <h3 className="font-display font-semibold hover:text-neon-light transition-colors line-clamp-1">
                                                            {item.name}
                                                        </h3>
                                                    </Link>
                                                    <p className="text-xs text-ink/40 mt-0.5">{item.brand}</p>
                                                </div>
                                                <button
                                                    onClick={() => removeFromCart(item.id)}
                                                    className="w-9 h-9 rounded-full glass flex items-center justify-center text-ink/40 hover:text-danger hover:border-danger/40 transition-all duration-300 shrink-0"
                                            ...
                                            <div className="flex items-end justify-between mt-4">
                                                <div className="flex items-center gap-3 rounded-full glass px-2 py-1.5">
                                                    <button
                                                        onClick={() => updateQty(item.id, item.qty - 1)}
                                                        className="w-7 h-7 rounded-full flex items-center justify-center text-ink/60 hover:text-ink hover:bg-ink/10 transition-colors"
                                                    ...
                                                    <motion.span
                                                        ...
                                                        initial={{ scale: 1.2 }}
                                                        animate={{ scale: 1 }}
                                                        className="text-sm font-mono font-semibold w-6 text-center"
                                                    >
                                                        {item.qty}
                                                    </motion.span>
                                                    <button
                                                ...
                                                >
                                                    <p className="font-display text-lg font-bold text-gradient-static">
                                                        ${(item.price * item.qty).toLocaleString()}
                                                    </p>
                                                    {item.qty > 1 && (
                                                        <p className="text-[10px] text-ink/30">${item.price.toLocaleString()} each</p>
                                                    )}
                                                </motion.div>
                                            </div>
                                        </div>
